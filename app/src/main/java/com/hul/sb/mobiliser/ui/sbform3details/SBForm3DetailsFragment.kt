...
import com.hul.databinding.FragmentSBForm3DetailsBinding
import com.hul.sb.SBDashboardComponent
import com.hul.sb.mobiliser.ui.sbform1details.SBForm1DetailsFragment
import com.hul.sb.mobiliser.ui.sbform2details.SBForm2DetailsViewModel
...
class SBForm3DetailsFragment : Fragment(), ApiHandler, RetryInterface {

    private var _binding: FragmentSBForm3DetailsBinding? = null
    ...
    @Inject
    lateinit var sbForm3DetailsViewModel: SBForm3DetailsViewModel
    ...
    override fun onCreateView(
        ...
        _binding = FragmentSBForm3DetailsBinding.inflate(inflater, container, false)
        ...
        sbForm3DetailsViewModel.projectInfo.value = Gson().fromJson(
            requireArguments().getString(ARG_CONTENT2),
            ProjectInfo::class.java
        )

        binding.viewModel = sbForm3DetailsViewModel
        return root
    }
    ...
    private fun visitsDataModel(): RequestModel {
        return sbForm3DetailsViewModel.projectInfo.value?.visit_id?.let {
            RequestModel(
    ...
    override fun onApiSuccess(o: String?, objectType: Int) {
        when (ApiExtentions.ApiDef.entries[objectType]) {
            ...
            ApiExtentions.ApiDef.GET_VISIT_DATA -> {
                ...
                sbForm3DetailsViewModel.visitData.value = Gson().fromJson(
                    model.getJSONObject("data").toString(),
                    GetVisitDataResponseData::class.java
                )
                Log.d("Nitin", requireArguments().getString(LOCAL_DATA)!!)
                if(sbForm3DetailsViewModel.visitData.value!!.visit_3 == null)
                {
                    val requestModel = Gson().fromJson(requireArguments().getString(LOCAL_DATA), RequestModel::class.java)
                    sbForm3DetailsViewModel.visitData.value!!.visit_3 = requestModel.visitData
                }
                if (sbForm3DetailsViewModel.visitData.value?.visit_3?.visit_image_1?.value != null) {
                    loadImage(
                        sbForm3DetailsViewModel.visitData.value?.visit_3?.visit_image_1!!.value.toString(),
                        binding.img1, binding.llImg1
                    )
                }

                if (sbForm3DetailsViewModel.visitData.value?.visit_3?.visit_image_2?.value != null) {
                    loadImage(
                        sbForm3DetailsViewModel.visitData.value?.visit_3?.visit_image_2!!.value.toString(),
                        binding.img2, binding.llImg2
                    )
                }
                Log.d("sbForm3DetailsViewModel.visitData", "onApiSuccess: ${sbForm3DetailsViewModel.visitData.value?.visit_3?.visit_image_3?.value}")

                if (sbForm3DetailsViewModel.visitData.value?.visit_3?.visit_image_3?.value != null) {
                    loadImage(
                        sbForm3DetailsViewModel.visitData.value?.visit_3?.visit_image_3!!.value.toString(),
